 ...
 * operations then gives options to name, save as and apply to one of two preset
 * buttons
 * 
 * </p>
 *
...
public class MacroActions {
    ...
    // Added here as many methods need these names and path and only requires the
    // scanner to be called once
    ...
    /**
     * <p>
     * Creates the menu bar, initializes variables and creates a path for the read
     * and write features
     * </p>
     */
    public MacroActions() {
    ...
    /**
     * <p>
     * Loads the actions menu with all the drop down options. Has been relocated
     * from MacroActions
    ...
    public void ReloadPresets() {
        ...
        // calls the loadPresets method to load in any presets and flag the presets as
        // true to allow menus to be created for them
        loadPresets();
        // creates instances of each macro menu option
        actions.add(new MacroStart(LanguageSettings.getTranslated("startMacro"),
                new ImageIcon("src/images/record_small.png"), LanguageSettings.getTranslated("startMDesc"),
                Integer.valueOf(KeyEvent.VK_G)));
        actions.add(new MacroLoad(LanguageSettings.getTranslated("loadFromFile"), null,
                LanguageSettings.getTranslated("loadMDesc"), Integer.valueOf(KeyEvent.VK_G)));
        // checks to make sure there is a preset in the txt file before loading it to
        // the actions menu
        if (preset1check == true) {
            actions.add(new Macro1(LanguageSettings.getTranslated("preset1") + " (" + Preset1Name + ")", null,
                    LanguageSettings.getTranslated("presetDesc"), Integer.valueOf(KeyEvent.VK_G)));
        }
        if (preset2check == true) {
            actions.add(new Macro2(LanguageSettings.getTranslated("preset2") + " (" + Preset2Name + ")", null,
                    LanguageSettings.getTranslated("presetDesc"), Integer.valueOf(KeyEvent.VK_G)));
        }
    }
    ...
    /**
     * <p>
     * Called by any action in Andie that shouldnt be running when recording is in
     * progress
    ...
    public static void macroRunningMsg() {
        ...
        JOptionPane.showOptionDialog(null, LanguageSettings.getTranslated("recordingInProgressMsg"),
                LanguageSettings.getTranslated("recordingInProgress"),
    ...
    /**
     * <p>
     * Used to check if a recording is currently running to stop actions that can
    ...
    /**
     * <p>
     * Used for EditableImage.apply() along side MacroActions.isRecording() allows
    ...
    public void changePreset(String presetName, Stack<ImageOperation> loadedOps) {
        ...
        String[] imageExtensions = { LanguageSettings.getTranslated("preset1"),
                LanguageSettings.getTranslated("preset2"), LanguageSettings.getTranslated("cancel"), }; // valid image
                                                                                                        // extensions to
                                                                                                        // be checked
        int presetChosen = JOptionPane.showOptionDialog(null, LanguageSettings.getTranslated("selectPreset"),
                LanguageSettings.getTranslated("presets"),
                JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE, null, imageExtensions, imageExtensions[0]);

        try {
            // creates a new scanner to make variables for what is already in the Presets
            // txt file
            ...
            sb.append(preset2);
            // takes the chosen preset and makes a string to replace the old file with the
            // new preset included
            switch (presetChosen) {
                case 0:
    ...
    public void loadPresets() {
        ...
        // attepts to read the Presets text file and if presets are avalible will load
        // them into the class
        try {
            ...
            // checks to make sure there is something in the file then procedes to load in
            // the first preset
            if (subMac.length() > 0) {
                preset1check = true;
                try {
            ...
            sc.close();
            // Displays an error message if the preset fails to load, will only happen when
            // a user edits the preset.txt file cannot happen while using the ANDIE
            // interface.
            ...
            if (result == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    ...
                    try {
                        ...
                        String[] imageExtensions = { LanguageSettings.getTranslated("apply"),
                                LanguageSettings.getTranslated("presets"), LanguageSettings.getTranslated("cancel"), }; // valid
                                                                                                                        // image
                                                                                                                        // extensions
                                                                                                                        // to
                                                                                                                        // be
                                                                                                                        // checked
                        int applyOrPreset = JOptionPane.showOptionDialog(null,
                                LanguageSettings.getTranslated("selectLoadAction"),
                                LanguageSettings.getTranslated("loadFromFile"), JOptionPane.DEFAULT_OPTION,
                                JOptionPane.PLAIN_MESSAGE, null, imageExtensions, imageExtensions[0]);
                    ...
                    // should be no reason for this to try to fail unless files are missing from
                    // Andie
                } catch (Exception ex) {
                    System.exit(1);
                }
    ...
    public class MacroStart extends ImageAction {
         ...
         * Simply changes the recording boolean to allow any EditableImage.apply() calls
         * to be recorded in the macro Stack
         * </p>
         *
         * @param e The event triggering this callback.
         */
        public void actionPerformed(ActionEvent e) {
            ...
            if (target.getImage().hasImage()) {
                ...
                if (isRecording() == false) {
                    ...
                    int option = JOptionPane.showOptionDialog(target.getParent(),
                            LanguageSettings.getTranslated("macroStartMsg"),
                            LanguageSettings.getTranslated("marcoStartTitle"), JOptionPane.OK_CANCEL_OPTION,
                            JOptionPane.QUESTION_MESSAGE, (Icon) getValue(Action.LARGE_ICON_KEY), new String[] {
                                    LanguageSettings.getTranslated("ok"), LanguageSettings.getTranslated("cancel") },
                            null);
                    ...
                    // asks user for the name of the recorded macro and creates a file path to save
                    // to
                    JPanel userAskPanel = new JPanel();
                    String userValue = JOptionPane.showInputDialog(userAskPanel,
                            LanguageSettings.getTranslated("nameMacro"));
                    ...
                    // makes a new instance of the current marco stack so the next start of macro
                    // recording does not continue from the end of this one
                    CurrentMacroStack = new Stack<ImageOperation>();
    ...
    public class Macro1 extends ImageAction {
        ...
        Macro1(String name, ImageIcon icon, String desc, Integer mnemonic) {
            ...
            KeyStroke f1 = KeyStroke.getKeyStroke(KeyEvent.VK_F1,
         ...
         * 
         * <p>
         * This method applys the macro currently loacted in the preset1 loaction to the
         * image
         * </p>
         * 
         * @param e The event triggering this callback.
    ...
    public class Macro2 extends ImageAction {
        ...
        Macro2(String name, ImageIcon icon, String desc, Integer mnemonic) {
            ...
            KeyStroke f2 = KeyStroke.getKeyStroke(KeyEvent.VK_F2,
                    Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx(), enabled);
         ...
         * 
         * <p>
         * This method applys the macro currently loacted in the preset2 loaction to the
         * image
         * </p>
         * 
         * @param e The event triggering this callback.
