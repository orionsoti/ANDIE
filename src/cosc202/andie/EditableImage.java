...
class EditableImage {
     ...
     * if the user doesn't enter a valid extension an option box will pop up
     * prompting the selection of a valid file type
     * 
     * @param imageFilename the file location to export the image to.
     * @throws Exception if there is no input or something happens during writing
     *                   the file.
     */
    public void exportImage(String imageFilename) throws Exception {
        ...
        if (!extensionCheck) {
            /// if the extension is invalid an option dialog will prompt the user to select
            /// a valid extension
            int extensionChoice = JOptionPane.showOptionDialog(null, LanguageSettings.getTranslated("selectFormat"),
                    LanguageSettings.getTranslated("format"), JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE,
                    null, imageExtensions, imageExtensions[0]);
     ...
     * Also checks if a macro is currently recording and if so, passes the operation
     * through to a Macro stack.
     * </p>
     * 
     ...
     * 
     * <p>
     * Implemented this way to allow for the undo and redo to work with each
     * operation applied within the macro also.
     * Once added to the stack the image call reefresh to refresh the new stack with
     * current operations and the macro added to thee end
     * 
     * @param macroOps the stack of Macro operations to be added to the current ops
     *                 file.
     *                 </p>
     */
